...
## [v0.75.4] — 2026-09-12
### Fixed
- 🔴 **元神家默认落在 macOS 受保护目录，导致常驻守护起不来**：分身常驻是 launchd LaunchAgent 后台拉起的，而 macOS TCC 把「桌面/文稿/下载」列为受保护目录，守护进程拿不到权限时会**直接卡死不启动**（实测：`home=~/Desktop/元神` 时 launchd 启动后**永不监听 8002**，但前台手动跑却完全正常——极具迷惑性）。
  - 默认家目录由 `~/Desktop/元神` 改为 **`~/元神`**（主目录根部，非 TCC 受保护，仍可见可管理）；
  - `/api/meta/home` 新增 `tcc_risk` 字段：当前家若落在受保护目录下会返回 `true`，前端可据此提示用户改路径；
  - 新增 `_home_under_tcc_protected()` 判定桌面/文稿/下载/影片/图片/公共目录。
- 顺带修掉本机一处真实隐患：元神家此前指向 `/tmp/fenshen_mh3`（系统临时目录，会被定期清空，存有 4.3MB 真实数据含 75 个项目），已合并迁移到持久路径（合并前完整备份，未删除任何文件）。
### Added
- `tests/test_meta_home_tcc.py`：锁死「受保护目录必标记 / 默认路径不受保护 / 不回退到桌面」（CI 必过）。

## [v0.75.3] — 2026-09-11
### Added
- **执行被拦 → 用户可见 + 可放行（补齐主动性的最后一块）**：此前「写文件越界 / 宪法代码护栏 / 系统确认框不可用 / 授权弹窗超时」只把一句话返给**角色**，用户在界面上完全看不到，表现就是「被拦住了就没声了」。现在：
  - 每次拦截都会向项目群聊落一条**用户可见**的说明（原因 + 目标 + 建议 + block id）；
  - 在群聊里回复「允许一次 / 放行 / 授权执行」即放行，元神会带着「用户已授权，请重试」的上下文重跑该动作；
  - 授权按 `(机器 kind, 具体目标)` 精确匹配，**一次性**、**10 分钟有效**、用完即废，绝不泛化放行；
  - 新增接口 `GET /api/meta/blocks`（列出待处理阻塞）与 `POST /api/meta/block/{bid}/allow`（放行），供前端后续挂「允许一次」按钮。
### Fixed
- **授权键不一致导致「授权了却仍被拦」**：`_notify_blocked` 记录的是中文标签，而放行检查用的是机器 kind，两者对不上。现已拆分为 `kind`（机器键，用于匹配）与 `label`（给用户看的中文名），并写进注释防止再犯。
### Added（测试）
- `tests/test_block_authorize.py`：锁死「拦截 → 可见 → 授权 → 重试成功 → 授权即用即废 → 过期失效」整条链（CI 必过）。

## [v0.75.2] — 2026-09-10
### Fixed
- **上限是「真硬上限」**：`_charge_request` 原用 `>` 判定，导致设 `request_max_calls=3` 时实际会打 **4** 次（超出 1 次）。改为 `>=`，达到即置 exceeded、下一次调用在入口被拦，实测调用数 = 上限值。
- **🔴 被截断却宣布成功（诚实性漏洞，实测发现）**：单次请求预算耗尽时，角色产出会退化成那句上限提示，但判定环节仍可能把它判成「完成」，于是群里同时出现「⛔ 已达上限」与「✅ 最终验收通过、项目已交付」。现角色判定强制优先检查预算状态：一旦触限即判 `review`（"单次请求已达调用上限，产出可能不完整 —— 转人工复核，不判达标"），绝不判 done。
- **上限设置项调不动（空头支票）**：触限提示引导用户去「设置」调高 `request_max_calls` / `request_max_tokens`，但 `/api/meta/settings` 是显式白名单、不收这两个键——提示形同死路。现已放行（`request_max_calls` 1~500、`request_max_tokens` 1000~5,000,000，带整数与范围校验），并在 `GET /api/meta/settings` 回显。
### Added
- `tests/test_request_budget.py` 增补「被截断的任务绝不判 done」的静态回归门禁。
- 端到端实测脚本（隔离引擎 + 生产库副本）验证：设上限 3 → 实际调用 **3** 次、任务落 `review`、无「验收通过」矛盾消息。
