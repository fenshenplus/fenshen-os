...
> 当前版本：**v5.8**（`0.58.0` / release `v5.8`，四项重构收口 + 设计规范库 + 官网推送）
> 基于：`v4.1`（git `f5b1929`）
> 分支：`fix/v4.0-audit`
> 最后更新：2026-08-14
...
| **v5.8** | 2026-08-14 | tag `v5.8`（待打） | **⏳ 待签字** | 四项重构收口：P0-A 元神PM定位+蒸馏绑定 ✅、P0-B 磨skill分段 ✅、P1 双维度存储（项目/成员/模块文件夹+点模块开文件+旧项目迁移）✅、P2 UI 设计规范库（apple_hig/wechat，建项自选无默认，注入角色执行）✅；安装包 v5.8 macOS dmg + 官网推送；冒烟 76/0 | — | — | — | — | — | — | — |
