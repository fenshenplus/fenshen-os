...
> 当前版本：**v5.0**（`0.50.0` / release `v5.0`，**2026-08-11 已签字**）
> 基于：`v4.1`（git `f5b1929`）
> 分支：`fix/v4.0-audit`
> 最后更新：2026-08-10
...
| **v5.0** | 2026-08-11 | 待打 tag v5.0 | **✅ 已签字（2026-08-11 岳衡「我签」）** | 移动端体验闭环：异步派单+进度轮询 / PWA 可安装离线 / 移动端看板快捷操作 / 手机默认群聊；WAL 并发写修复；演示视频 v4 重录 | 自主闭环：立项自动拆解（/plan）+ 看板完成度 + 阶段自动流转 + 团队自主推进循环（元神不提问）+ 派单并行化；DeepSeek 迁移 v4-flash + 关闭思考模式；安装包 v4.2 已上线官网 |
