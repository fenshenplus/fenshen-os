#!/usr/bin/env bash
# 分身 v6.3 · 一键启动（双击运行）
# 双击本文件即可在本地启动分身，并自动打开浏览器（首次会联网安装依赖）
cd "$(cd "$(dirname "$0")" && pwd)"
...
if curl -s -m 1 http://127.0.0.1:8002/api/health >/dev/null 2>&1; then
  ...
  # 局域网模式：用 FENSHEN_ALLOW_LAN=1 开启（访问需令牌，仅限可信网络）
  [ "$FENSHEN_ALLOW_LAN" = "1" ] && echo "⚠️  局域网模式已开启，访问需令牌（data/.auth_token）"
  nohup "$PY" start.py > /tmp/fenshen-v1.log 2>&1 &
  # 等待端口可用
